...
struct state {
        ...
        struct gluster_url *gluster_url2;
...
usage ()
{
        printf ("Usage: %s [OPTION]... URL\n"
                ...
                "Examples:\n"
                "  gflink glfs://localhost/groot/path/to/file glfs://localhost/groot/path/to/link\n"
                "        Create link to /path/to/file at /path/to/link on the Gluster volume\n"
                "        of groot on host localhost to standard output.\n"
                "  gfcli (localhost/groot)> link /file /link\n"
                "        In the context of a shell with a connection established,\n"
                "        link the file on the root of the Gluster volume groot\n"
                "        on localhost at /link.\n",
...
parse_options (int argc, char *argv[], bool has_connection)
{
        ...
        if ((argc - option_index) < 3) {
                ...
                state->url = strdup (argv[argc - 2]);
                if (state->url == NULL) {
                        error (0, errno, "strdup");
                        goto out;
                }
                state->url2 = strdup (argv[argc - 1]);
                if (state->url2 == NULL) {
                        error (0, errno, "strdup");
                        goto out;
                }
                ...
                if (has_connection) {
                        ...
                        state->gluster_url2 = gluster_url_init ();
                        if (state->gluster_url2 == NULL) {
                                error (0, errno, "gluster_url_init");
                                goto out;
                        }
                        
                        state->gluster_url->path = strdup (argv[argc - 2]);
                        if (state->gluster_url->path == NULL) {
                                error (0, errno, "strdup");
                                goto out;
                        }
                        state->gluster_url2->path = strdup (argv[argc - 1]);
                        if (state->gluster_url2->path == NULL) {
                                error (0, errno, "strdup");
                                goto out;
                        }
                ...
                ret = gluster_parse_url (argv[argc - 2], &(state->gluster_url));
                if (ret == -1) {
                        error (0, EINVAL, "%s", state->url);
                        goto err;
                }else{
                        if(state->gluster_url->path[0]=='/')
                                (state->gluster_url->path)++;
                }
                state->gluster_url->port = port;
                ret = gluster_parse_url (argv[argc - 1], &(state->gluster_url2));
                if (ret == -1) {
                        error (0, EINVAL, "%s", state->url2);
                        goto err;
                }else{
                        if(state->gluster_url2->path[0]=='/')
                                (state->gluster_url2->path)++;
                }
                state->gluster_url2->port = port;
        }
...
link_without_context ()
{
        ...
        ret = glfs_symlink(fs, state->gluster_url->path, state->gluster_url2->path);
        if (ret == -1) {
                goto out;
        }
...
int
do_link (struct cli_context *ctx)
{
        int argc = ctx->argc;
        char **argv = ctx->argv;
        ...
        if (ctx->fs) {
                ...
                ret = glfs_symlink (ctx->fs, state->gluster_url->path,state->gluster_url2->path);
        } else {
                ...
                ret = link_without_context ();
        }

out:
        if (state) {
                gluster_url_free (state->gluster_url);
                gluster_url_free (state->gluster_url2);
                free (state->url);
        }
